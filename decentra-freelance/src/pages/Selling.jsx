...
function Selling() {
  ...
  async function loadBuyer() {
    ...
    for (let index = 1; index <= noOfuser.toString(); index++) {
      ...
      if (user.account.toLowerCase() == account[0]) {
        ...
        console.log(
          "🚀 ~ file: Selling.jsx:116 ~ loadBuyer ~ metadata:",
          metadata
        );
  ...
  async function uploadBuyerToStorage() {
    ...
    console.log(
      "🚀 ~ file: Selling.jsx:239 ~ uploadBuyerToStorage ~ uri:",
      uri
    );
    const urlRemoved = uri.replace("ipfs://", "");
    const url = urlRemoved.replace("/0", "");
    console.log(url);
    const profileSet = await skillswap.setProfile(url, 1);
    console.log(
      "🚀 ~ file: Selling.jsx:243 ~ uploadBuyerToStorage ~ profileSet:",
      profileSet
    );
    setLoading(false);
  }
                ...
                <ProfileContent className={` ${isOpen ? "" : "gridOneColumn"}`}>
                  {displayProfile.map((profileData, idx) => (
                    ...
                    >
                      <YourDetail
                        className={`noSidebar ${
                          isOpen ? "halfWidth" : "fullWidth"
                        }`}
                        onClick={openSidebar}
                      >
                        <div className={` ${isOpen ? "noDisplay" : ""}`}>
                          <PPContainer>
                            <img
                      ...
                      ></CloseBtn>
                      <YourDetail className="noShowProfile">
                        <PPContainer>
                          <img
                            src={`https://gateway.ipfscdn.io/ipfs/${profileData.image}`}
                            alt={profileData.name}
                ...
                <ProfileContent>
                  {displayBuyer.map((profileData, idx) => (
                    <div key={idx}>
                      ...
                      <div>{profileData.profileTitle}</div>
                      <div>{profileData.description}</div>
                      <div>{profileData.country}</div>
                      <div>{profileData.twitterLink}</div>
                      <div>{profileData.urlS}</div>
                    </div>
                  ))}
                </ProfileContent>
              )}
            ...
            <div>
              {isUserSelected ? (
                <div>
                        ...
                        {firstPage ? (
                          ...
                          <GigContainer>
                            ...
                            >
                              ...
                              <option value="Collection Artist">
                                Collection Artist
                              </option>
                            ...
                            <WarningTxt style={{ marginTop: "40px" }}>
                              ...
                              market. Lastly, Bull Market Price is best for gigs
                              that require high prices when the market is up and
                              lower prices when it is down.
                            </WarningTxt>
                  ...
                  {selectUser == "client" && (
                    <div>
                      <form onSubmit={makeBuyerAccount}>
                        <ProfileContainer>
                          <h2>About Yourself</h2>
                          <WarningTxt>
                            <span>WARNING:</span> Any changes/ edits to a user
                            ...
                            account and fill out all necessary information in
                            one go to avoid any unnecessary gas fees. If you
                            want to create multiple gigs on SkillSwap, you must
                            use a different ETH wallet address for each one.
                ...
                <SelectUser>
                  <div>
                    <div onClick={() => setSelectUser("freelancer")}>
                      Freelancer
                    </div>
                    <div onClick={() => setSelectUser("client")}>Client</div>
                  </div>
                  {selectUser != "" && (
                    <div>
